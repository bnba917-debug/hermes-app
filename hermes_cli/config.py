...
_UNSET = object()
_USER_CONFIG_OVERRIDE: ContextVar[object] = ContextVar(
    "_USER_CONFIG_OVERRIDE", default=_UNSET
)
_USER_ENV_OVERRIDE: ContextVar[object] = ContextVar("_USER_ENV_OVERRIDE", default=_UNSET)


def set_user_config_override(config: Optional[Dict[str, Any]]) -> Token:
    """Context-local config dict for per-task scoping (e.g. App Gateway DB users)."""
    value: object = _UNSET if config is None else config
    return _USER_CONFIG_OVERRIDE.set(value)


def reset_user_config_override(token: Token) -> None:
    _USER_CONFIG_OVERRIDE.reset(token)


def get_user_config_override() -> Optional[Dict[str, Any]]:
    override = _USER_CONFIG_OVERRIDE.get()
    if override is _UNSET or override is None:
        return None
    return override  # type: ignore[return-value]


def set_user_env_override(env: Optional[Dict[str, str]]) -> Token:
    """Context-local env secrets for per-task scoping (e.g. App Gateway DB users)."""
    value: object = _UNSET if env is None else env
    return _USER_ENV_OVERRIDE.set(value)


def reset_user_env_override(token: Token) -> None:
    _USER_ENV_OVERRIDE.reset(token)


def get_user_env_override() -> Optional[Dict[str, str]]:
    override = _USER_ENV_OVERRIDE.get()
    if override is _UNSET or override is None:
        return None
    return override  # type: ignore[return-value]
...
DEFAULT_CONFIG = {
    ...
    # Shared storage backends (session DB, optional PostgreSQL).
    # When ``session_backend`` is ``auto`` and ``postgres_url`` is set, Hermes
    # uses PostgreSQL for ~/.hermes session/message storage instead of state.db.
    "storage": {
        "postgres_url": "",
        "session_backend": "auto",
        "kanban_backend": "auto",
        "cron_backend": "auto",
        # PG connections per process (use 32–64 for 100+ concurrent agents).
        "postgres_pool_size": 48,
        # When true, refuse SQLite fallbacks for session/kanban/cron and app_gateway stores.
        "postgres_only": False,
    },

    # Multi-user App Gateway (plugins/app_gateway) — JWT + SSE for mobile/web.
    # Start with: hermes app-gateway start
    "app_gateway": {
        "enabled": False,
        "host": "127.0.0.1",
        "port": 8787,
        ...
        "platform_toolset": "app_gateway",
        "user_registry_backend": "auto",
        "postgres_only": False,
        "require_redis": False,
        "trusted_proxy_ips": [],
        ...
        "vector_memory_backend": "auto",
        "rate_limit_rpm": 60,
        "proxy_to_api_server": False,
        ...
        "minio_bucket": "hermes-workspaces",
        "minio_secure": False,
        "minio_prefix": "workspaces",
        "workspace_minio_async_upload": True,
        "workspace_upload_workers": 8,
        "workspace_search_prefetch_max_files": 100,
        "workspace_cache_gc_enabled": True,
        "workspace_cache_ttl_hours": 72,
        "workspace_cache_max_mb": 256,
        "workspace_cache_gc_interval_seconds": 300,
        "auth_sms_per_ip_per_hour": 30,
        "auth_sms_per_phone_per_day": 10,
        "auth_login_failures_per_phone": 15,
        "sms_captcha_enabled": True,
        "sms_captcha_ttl_seconds": 300,
        "sms_captcha_max_operand": 20,
        "data_retention_days": 365,
        "data_retention_interval_hours": 24,
        "workspace_upload_max_retries": 3,
        ...
        "require_onboarding_before_chat": True,
        "onboarding_models": [],
    },

    # Standalone web admin for app users, public skills, and billing.
    "app_admin": {
        "enabled": False,
        "host": "127.0.0.1",
        "port": 8790,
...
OPTIONAL_ENV_VARS = {
    ...
    # ── App Gateway SMS (secrets in .env; sign/template in config.yaml) ──
    "ALIYUN_SMS_ACCESS_KEY_ID": {
        "description": "Aliyun SMS AccessKey ID (app_gateway auth_mode: aliyun)",
        "prompt": "Aliyun SMS AccessKey ID",
        "password": False,
        "category": "tool",
    },
    "ALIYUN_SMS_ACCESS_KEY_SECRET": {
        "description": "Aliyun SMS AccessKey secret",
        "prompt": "Aliyun SMS AccessKey secret",
        "password": True,
        "category": "tool",
    },
    "TENCENT_SMS_SECRET_ID": {
        "description": "Tencent Cloud SMS SecretId (auth_mode: tencent)",
        "prompt": "Tencent SMS SecretId",
        "password": False,
        "category": "tool",
    },
    "TENCENT_SMS_SECRET_KEY": {
        "description": "Tencent Cloud SMS SecretKey",
        "prompt": "Tencent SMS SecretKey",
        "password": True,
        "category": "tool",
    },
    "TWILIO_ACCOUNT_SID": {
        "description": "Twilio account SID (auth_mode: twilio)",
    ...
    "APP_GATEWAY_SMS_WEBHOOK_URL": {
        "description": "HTTP webhook URL for custom SMS bridge (auth_mode: http)",
        "prompt": "App Gateway SMS webhook URL",
        "password": False,
        "category": "tool",
    },
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        override = get_user_config_override()
        if override is not None:
            config = copy.deepcopy(DEFAULT_CONFIG)
            config = _deep_merge(config, copy.deepcopy(override))
...
def load_env() -> Dict[str, str]:
    ...
    """
    override = get_user_env_override()
    if override is not None:
