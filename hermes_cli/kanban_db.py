...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    When ``storage.kanban_backend`` is ``auto`` or ``postgres`` and
    ``storage.postgres_url`` is set, returns a PostgreSQL-backed connection
    (one schema per board) instead of SQLite.
    """
    if db_path is None and not os.environ.get("HERMES_KANBAN_DB", "").strip():
        try:
            from agent.session_storage.config import resolve_kanban_backend

            backend, dsn = resolve_kanban_backend(db_path)
            if backend == "postgres" and dsn:
                ...
                return postgres_connect(dsn, board=slug or DEFAULT_BOARD)
        except Exception as exc:
            _log.warning(
                "PostgreSQL kanban backend unavailable, using SQLite: %s", exc
            )
