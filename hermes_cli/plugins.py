...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        for manifest in winners.values():
            ...
            # Bundled app-gateway: follow app_gateway.enabled in config.yaml so
            # ``hermes app-gateway start`` works after setup without a separate
            # ``hermes plugins enable app-gateway`` step.
            if not is_enabled and manifest.name == "app-gateway":
                try:
