...
def _is_app_gateway_file_session() -> bool:
    try:
...
def _resolve_path_for_task(filepath: str, task_id: str = "default") -> Path:
    ...
    if _is_app_gateway_file_session():
        from plugins.app_gateway.workspace_paths import resolve_app_gateway_path

        resolved, err = resolve_app_gateway_path(filepath, task_id=task_id)
        if err:
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        if not Path(_resolved).is_file():
            try:
                from plugins.app_gateway.workspace_sync import prefetch_workspace_file

                prefetch_workspace_file(path, task_id)
            except Exception:
                pass
    ...
    try:
        ...
        if _resolved is None:
            ...
            if not result_dict.get("error"):
                try:
                    from plugins.app_gateway.workspace_sync import sync_workspace_relative_path

                    sync_workspace_relative_path(path, task_id)
                except Exception:
                    pass
            return json.dumps(result_dict, ensure_ascii=False)
            ...
            if not result_dict.get("error"):
                for _p in _paths_to_check:
                    ...
                    try:
                        ...
                        sync_workspace_relative_path(_p, task_id)
                    except Exception:
                        pass
...
def search_tool(pattern: str, target: str = "content", path: str = ".",
    ...
    try:
        ...
        try:
            from plugins.app_gateway.workspace_sync import prefetch_workspace_search

            prefetch_workspace_search(
                path,
                task_id,
                pattern=pattern,
                target=target,
