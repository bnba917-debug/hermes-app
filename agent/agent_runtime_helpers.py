...
def _coerce_api_message_content(content: Any) -> Any:
    """Ensure ``content`` is API-safe for OpenAI-compatible providers.

    Postgres session replay JSON-decodes tool payloads into dicts. Moonshot/Kimi
    rejects bare objects — tool results must be strings; multimodal user content
    may remain a list of typed parts.
    """
    if content is None:
        return ""
    ...
    if isinstance(content, list):
        if content and all(
            isinstance(part, dict) and part.get("type") for part in content
        ):
            return content
        return json.dumps(content, ensure_ascii=False)
    if isinstance(content, dict):
        # Single typed part should be wrapped; otherwise stringify JSON payloads.
        if content.get("type"):
            return [content]
        return json.dumps(content, ensure_ascii=False)
    return str(content)
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for msg in messages:
        ...
        if "content" in msg:
            patched = dict(msg)
            patched["content"] = _coerce_api_message_content(msg.get("content"))
