...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    lightweight, unless a context-local config override is active.
    """
    skills_cfg: dict = {}
    override = None
    try:
        from hermes_cli.config import get_user_config_override
    ...
    if override is not None:
        raw = override.get("skills")
        skills_cfg = raw if isinstance(raw, dict) else {}
    else:
...
_EXTERNAL_DIRS_OVERRIDE_UNSET = object()
_EXTERNAL_DIRS_OVERRIDE: ContextVar[object] = ContextVar(
    "_EXTERNAL_DIRS_OVERRIDE", default=_EXTERNAL_DIRS_OVERRIDE_UNSET
)


def set_external_skills_dirs_override(dirs: Optional[List[Path]]) -> Token:
    """Context-local external skill dirs (e.g. App Gateway DB-backed users)."""
    value: object = _EXTERNAL_DIRS_OVERRIDE_UNSET if dirs is None else dirs
    return _EXTERNAL_DIRS_OVERRIDE.set(value)


def reset_external_skills_dirs_override(token: Token) -> None:
    _EXTERNAL_DIRS_OVERRIDE.reset(token)


def get_external_skills_dirs_override() -> Optional[List[Path]]:
    override = _EXTERNAL_DIRS_OVERRIDE.get()
    if override is _EXTERNAL_DIRS_OVERRIDE_UNSET:
        return None
    return list(override)  # type: ignore[arg-type]
...
def get_external_skills_dirs() -> List[Path]:
    ...
    """
    override = get_external_skills_dirs_override()
    if override is not None:
