...
def test_sanitize_coerces_dict_tool_content_to_string():
    ...
    msgs = [
        ...
        },
        {
            "role": "tool",
            "tool_call_id": "call_1",
            "content": {"success": True, "name": "demo"},
        },
    ]
    out = sanitize_api_messages(msgs)
    assert isinstance(out[-1]["content"], str)
    assert '"success": true' in out[-1]["content"] or '"success": True' in out[-1]["content"]
