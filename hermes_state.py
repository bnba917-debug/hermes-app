...
class SessionDB(SessionStoreBase):
    """
    Session storage with FTS5 search (SQLite) or PostgreSQL when configured.

    Thread-safe for the common gateway pattern (multiple reader threads,
    single writer via WAL mode). Each method opens its own cursor.

    With ``storage.session_backend: auto`` and ``storage.postgres_url`` set
    (or ``app_gateway.postgres_url`` / ``HERMES_STORAGE_POSTGRES_URL``),
    ``SessionDB()`` uses PostgreSQL instead of ``~/.hermes/state.db``.
    Pass an explicit ``db_path`` to force SQLite (tests, custom paths).
    """
    ...
    def __new__(cls, db_path=None):
        ...
        try:
            ...
            backend, dsn = resolve_session_backend(db_path)
            if backend == "postgres" and dsn:
                from agent.session_storage.postgres_session_db import (
                    get_postgres_session_db,
                )

                return get_postgres_session_db(dsn)
        except Exception as exc:
            logger.warning(
                "PostgreSQL session backend unavailable, using SQLite: %s",
                exc,
            )
        return super().__new__(cls)

    def __init__(self, db_path=None):
        if type(self) is not SessionDB:
            return
...
# ── Process-wide session store (gateway / api_server / app_gateway) ──

_shared_sqlite_lock = threading.Lock()
_shared_sqlite_by_path: dict[str, "SessionDB"] = {}


def get_shared_session_db(db_path=None):
    """Return one SessionDB (or PostgresSessionDB) instance per process.

    Prefer this over ``SessionDB()`` in gateway hot paths so SQLite does not
    open a new connection per HTTP request.  Explicit ``db_path`` forces
    SQLite at that path (tests, custom locations).
    """
    if db_path is not None:
        ...
        with _shared_sqlite_lock:
            inst = _shared_sqlite_by_path.get(key)
            if inst is None:
                inst = SessionDB(db_path=Path(db_path) if not isinstance(db_path, Path) else db_path)
                _shared_sqlite_by_path[key] = inst
            return inst
    ...
    try:
        ...
        backend, dsn = resolve_session_backend(None)
        if backend == "postgres" and dsn:
            from agent.session_storage.postgres_session_db import get_postgres_session_db

            return get_postgres_session_db(dsn)
        if load_postgres_only_flag():
            reject_sqlite_backend(backend, component="session", dsn=dsn)
    except PostgresOnlyError:
        raise
    except Exception as exc:
        if load_postgres_only_flag():
            raise RuntimeError(
                f"session: postgres_only=true but PostgreSQL session backend failed: {exc}"
            ) from exc
        logger.warning(
            "PostgreSQL session backend unavailable for shared db, using SQLite: %s",
            exc,
        )

    key = str(DEFAULT_DB_PATH.resolve())
    if load_postgres_only_flag():
        from plugins.app_gateway.postgres_policy import PostgresOnlyError

        raise PostgresOnlyError(
            "session: postgres_only=true forbids SQLite session store fallback"
        )
    with _shared_sqlite_lock:
        inst = _shared_sqlite_by_path.get(key)
        if inst is None:
...
def reset_shared_session_stores() -> None:
    """Close and drop process-wide session store caches (tests, benchmarks)."""
    with _shared_sqlite_lock:
        for inst in _shared_sqlite_by_path.values():
            try:
                inst.close()
            except Exception:
                pass
        _shared_sqlite_by_path.clear()
    try:
        from agent.session_storage import postgres_session_db as _pg

        with _pg._instances_lock:
            for inst in _pg._instances.values():
                try:
                    inst.close()
                except Exception:
                    pass
            _pg._instances.clear()
    except Exception:
        pass
