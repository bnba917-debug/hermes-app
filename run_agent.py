...
class AIAgent:
    ...
    def _persist_session(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        if getattr(self, "save_session_log", True):
    ...
    def _flush_messages_to_session_db(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        try:
            ...
            gap = self._history_gap_messages(conversation_history)
            start_idx = len(conversation_history) if conversation_history else 0
            flush_from = max(start_idx, self._last_flushed_db_idx)
            to_write = gap + messages[flush_from:]
            if to_write:
                self._append_messages_to_session_db(to_write)
            self._last_flushed_db_idx = len(messages)
            self._refresh_session_db_row_cache()
        except Exception as e:
            logger.warning("Session DB append_message failed: %s", e)

    def _append_messages_to_session_db(self, msgs: List[Dict]) -> None:
        if not msgs:
            return
        rows = [self._prepare_message_for_db(msg) for msg in msgs]
        batch_fn = getattr(self._session_db, "append_messages_batch", None)
        if batch_fn:
            batch_fn(self.session_id, rows)
            return
        for row in rows:
            self._session_db.append_message(session_id=self.session_id, **row)

    def _refresh_session_db_row_cache(self) -> None:
        count_fn = getattr(self._session_db, "message_count", None)
        if not callable(count_fn):
            return
        try:
            self._session_db_rows = int(count_fn(self.session_id))
        except Exception:
            self._session_db_rows = None

    def _history_gap_messages(
        self, conversation_history: Optional[List[Dict]] = None
    ) -> List[Dict]:
        """Return conversation_history rows missing from the session DB (count-based)."""
        if not self._session_db or not conversation_history:
            return []
        hist_len = len(conversation_history)
        cached = getattr(self, "_session_db_rows", None)
        if cached is not None and hist_len <= cached:
            return []
        count_fn = getattr(self._session_db, "message_count", None)
        if not callable(count_fn):
            return []
        try:
            db_count = int(count_fn(self.session_id))
        except Exception:
            return []
        self._session_db_rows = db_count
        if hist_len <= db_count:
            return []
        return list(conversation_history[db_count:])

    def _backfill_history_gap_to_session_db(
        self, conversation_history: Optional[List[Dict]] = None
    ) -> None:
        """Append conversation_history rows missing from the session DB (count-based)."""
        gap = self._history_gap_messages(conversation_history)
        if gap:
            self._append_messages_to_session_db(gap)
            self._refresh_session_db_row_cache()

    def _prepare_message_for_db(self, msg: Dict) -> Dict[str, Any]:
    ...
    def _append_single_message_to_session_db(self, msg: Dict) -> None:
        self._session_db.append_message(
            session_id=self.session_id,
            **self._prepare_message_for_db(msg),
        )
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        """
        if not getattr(self, "save_session_log", True):
            return
