...
# Per-task working directory for file tools (app gateway sets per-user workspace).
...
def set_terminal_cwd(path: str):
    """Pin file-tool resolution to *path* for the current context (thread-safe)."""
    return _TERMINAL_CWD.set(str(path))


def reset_terminal_cwd(token) -> None:
    """Restore the previous terminal cwd context (from :func:`set_terminal_cwd`)."""
    _TERMINAL_CWD.reset(token)


def get_terminal_cwd(*, fallback_getcwd: bool = True) -> str:
    """Resolve the active working directory for relative file paths.

    Resolution order:
    1. Context variable (app gateway per-user ``workspace/``).
    2. ``os.environ['TERMINAL_CWD']`` (CLI / gateway bridge).
    3. ``os.getcwd()`` when *fallback_getcwd* is true.
    """
    import os
