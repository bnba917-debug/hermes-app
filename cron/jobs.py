...
def _postgres_cron_store():
    try:
        from agent.session_storage.config import resolve_cron_backend

        backend, dsn = resolve_cron_backend()
        if backend == "postgres" and dsn:
            from cron.postgres_store import get_postgres_cron_store

            return get_postgres_cron_store(dsn)
    except Exception as exc:
        logger.warning("PostgreSQL cron backend unavailable, using jobs.json: %s", exc)
    return None


def load_jobs() -> List[Dict[str, Any]]:
    """Load all jobs from storage."""
    pg = _postgres_cron_store()
    if pg is not None:
        return pg.load_jobs()
